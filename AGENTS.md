...
- **keyd SUPER-as-Cmd remap, real-hardware behavior unconfirmed**: `system/etc/keyd/default.conf` overrides only `c`/`v`/`k` inside keyd's implicit `[meta]` layer, remapping `super+c/v/k` → `ctrl+shift+c/v/k` (the exact combos Ghostty's `config` already binds to copy/paste/clear) at the evdev level, below Hyprland's compositor-level SUPER grab. **`[main]` also swaps the physical Alt and Super/Win keys system-wide** (`leftalt = leftmeta`, `leftmeta = leftalt`; `rightalt`/AltGr untouched for fcitx5/Unikey compose sequences) — so throughout this repo, "SUPER" means the thumb-reachable physical Alt key and "ALT" means the physical Win key, not the printed keycap; this affects every app, not just Hyprland. A prior version of this remap had `super+v`/`super+k` silently dead-on-arrival in Hyprland, because keyd converted them to `ctrl+shift+v/k` *before* Hyprland's compositor grab ever saw a plain `super+v`/`super+k` — colliding with Hyprland's own `SUPER+V` (clipboard-picker) and `SUPER+K` (focus-up) binds in `hyprland/.config/hypr/binds.lua`. Fixed by moving Hyprland's window-nav (focus + move, vim-style HJKL) off `mainMod` ("SUPER") onto a separate `navMod` ("ALT"), and moving clipboard-picker from `SUPER+V` to `SUPER+SHIFT+V` — `super+c` was never bound in Hyprland so needed no change. Still not verified against a physical keyboard for layout edge cases (e.g. non-US layouts), interaction with fcitx5's own Ctrl-based bindings, or whether keyd's `[main]` swap correctly feeds into the `[meta]`-layer c/v/k remap below it (i.e. whether `meta`/`alt` layer triggers follow the post-swap keycode, as keyd's docs claim for this pattern).
